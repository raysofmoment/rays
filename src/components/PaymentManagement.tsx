...
import { CheckCircle2, XCircle, Loader2, Calendar, IndianRupee, User, FileText, ExternalLink, Search, Clock, Download } from 'lucide-react';
          ...
          <button
            onClick={() => {
              const exportData = payments.map(p => ({
                'Client Name': p.clientName || 'Unknown',
                'Order ID': p.orderId || '-',
                'Amount': p.amount || 0,
                'Method': p.method || '-',
                'Date': p.date && isValid(new Date(p.date)) ? format(new Date(p.date), 'yyyy-MM-dd') : '-',
                'Status': p.status,
                'Created At': p.createdAt && isValid(new Date(p.createdAt)) ? format(new Date(p.createdAt), 'yyyy-MM-dd HH:mm') : '-',
                'Slip URL': p.slipUrl || '-'
              }));
              exportToExcel(exportData, 'Payments_List');
            }}
            className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-xl font-semibold hover:bg-purple-700 transition-colors shadow-sm text-sm"
          >
